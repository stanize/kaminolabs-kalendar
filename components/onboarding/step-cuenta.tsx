    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        ...
        <input type="password" placeholder="Repite la contraseña"      value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={loading}
          onKeyDown={(e) => e.key === "Enter" && handleSubmit()} className={inputClass} />
...
export function StepCuenta() {
  ...
  return view === "picker"
    ? <PickerView onGoogle={handleGoogle} onEmail={() => setView("register")} />
    : <RegisterView onSuccess={(name, email) => setEmailAuthed(name, email)} onBack={() => setView("picker")} />;
}
