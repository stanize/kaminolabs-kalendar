...
import { DAYS, HOURS } from "@/lib/onboarding/data";
import { useOnboardingStore } from "@/lib/onboarding/store";

export function StepHorario() {
  const schedule      = useOnboardingStore((s) => s.d.schedule);
  const setScheduleDay = useOnboardingStore((s) => s.setScheduleDay);

  return (
    <div className="flex flex-col gap-1">
      {DAYS.map((day) => {
        const h = schedule[day.id];
        return (
          <div
            key={day.id}
            className={clsx(
          ...
          >
            <label className="relative inline-flex h-5 w-[34px] shrink-0 cursor-pointer items-center">
              <input
                type="checkbox"
                checked={h.on}
                onChange={(e) => setScheduleDay(day.id, { on: e.target.checked })}
            ...
            <span className="flex-1 text-[14.5px] font-semibold">{day.label}</span>

            {h.on ? (
              <div className="flex items-center gap-2 text-[13px] text-ink-soft">
                <select
                  value={h.from}
                  onChange={(e) => setScheduleDay(day.id, { from: e.target.value })}
                ...
                <select
                  value={h.to}
                  onChange={(e) => setScheduleDay(day.id, { to: e.target.value })}
                  className="cursor-pointer rounded-[9px] border border-line bg-surface px-2.5 py-2 text-[13.5px] font-semibold text-ink outline-none focus:border-brand"
                >
                  {HOURS.map((hour) => (
                    <option key={hour} value={hour}>{hour}</option>
