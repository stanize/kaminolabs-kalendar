...
- Routes: `app/bookings/[slug]/page.tsx`, `app/bookings/cancel/[token]/page.tsx` (`app/bookings/confirm/[token]/page.tsx` REMOVED 2026-09 with the guest confirm-by-link flow)
...
- i18n: `lib/i18n/dictionaries/booking-page.ts`, `booking-result.ts` (its `confirm` section is now dead/unused text, kept rather than pruned — low-risk residue from the confirm-route removal)
- Depends on shared infra: email (`lib/email.ts` — confirm/cancel/owner-notify emails), auth (patient login step inside wizard)
- Gotchas: `lib/business/booking-url.ts` is SHARED with panel-business (booking link display) — changes here affect both. Timezone hardcoded `Europe/Madrid` in `lib/booking/slots.ts`. `ConfirmAuthModal` inside `booking-wizard.tsx` is a SEPARATE, self-contained login/register implementation — it does NOT use `components/auth/patient-login-form.tsx`, so any change to the shared patient login (e.g. forgot-password link, error copy) must be mirrored here by hand. Its forgot-password link does a hard `window.location.href` nav (not a soft `<Link>`) to `/forgot-password?from=patient&redirectTo=<current booking page path>` — computed inside the click handler, not render body, since this is SSR'd and `window` isn't available server-side. **Every normal-flow booking is confirmed immediately on submit now (2026-09, guest-immediate-confirm-with-clinic-followup, extended to also cover a patient's very first booking made mid-sign-up)** — the old 24h pending_confirmation/auto-expiry window, its confirm-by-link route, the sweep cron that expired it, AND `finalizeVerifiedPatientBookings`/`PatientBookingFinalizer` (the promote-on-verify step that subsumed) were all removed; `submitBooking` (`lib/actions/booking.ts`) now only produces `pending_confirmation` via admin tooling's `statusOverride` escape hatch. The email-verification gate this used to double as lives entirely on the ACCOUNT side instead (`app/patient/(protected)/layout.tsx`'s `PatientEmailVerificationGate` — see `patient-portal`): the booking itself is a one-shot confirmed action, but an unverified patient can't re-login and use the portal until they verify. `shouldSendOwnConfirmEmail` in `submitBooking` is the one remaining distinction between an unverified sign-up-mid-booking and everyone else — it skips its OWN receipt email specifically for that case, since Better Auth's `sendVerificationEmail` hook (`lib/auth.ts`) already sent one combined "confirm your email + here's your booking" email moments earlier; the booking's `status` is identical either way. `kalendar_bookings.clinic_reviewed_at` (nullable) is a standing, clinic-set follow-up flag layered on top of a guest booking's already-confirmed status — see `panel-calendar`'s gotchas.
...
- Lib: `lib/booking/patient-data.ts`, `lib/booking/patient-claim.ts` (new, 2026-09 — see below)
- Actions: `lib/actions/patient.ts`
- DB tables: `kalendar_patients` (now has `claim_checked_at`), reads `kalendar_bookings`
...
- Gotchas: `provisionPatient` assigns patient role AND upserts `kalendar_patients` — don't split these without checking both call sites. **"Unify guests and patients" (2026-09)**: `lib/booking/patient-claim.ts` — `claimExistingClientHistory(patientId, email)` links every unlinked `kalendar_clients` row (GLOBAL, across every business) matching that email to the patient, and backfills `patient_id` onto every one of that client's past `kalendar_bookings` too, so nothing about their pre-registration guest/walk-in history is lost. Called from two places: `claimGuestHistoryIfEligible()` (`lib/actions/patient.ts`) is called INLINE, server-side, at the top of `app/patient/(protected)/page.tsx` — BEFORE that page fetches bookings, so if it links anything, the first render already shows it (deliberately NOT a client component/useEffect, which caused a real render-then-refresh flash the first version had — see that function's own comment) — fires once per patient the first time their session shows `emailVerified: true` (`kalendar_patients.claim_checked_at` makes it a no-op after that); this can NOT live inside `provisionPatient()` itself, since that runs immediately after sign-up, before verification. AND `createBookingAsOwner` (`lib/actions/booking-owner.ts`, panel-calendar module) when staff types an email matching an existing verified patient. Both directions require the matched account's email to be verified (`findVerifiedPatientIdByEmail`, same file) — matching by an unverified/self-typed email would let someone claim a stranger's booking history.
...
- Gotchas: The "Clientes" tab (`calendar-bookings.tsx`, replaced the old "Pendientes" tab) is a flat list (no day grouping) surfacing guest bookings and first-time patients that warrant a closer look — filterable by `clientStatus`. As of 2026-09 (guest-immediate-confirm-with-clinic-followup) it spans the guest's FULL history, not just upcoming (`getClientRowBookings`, `lib/booking/owner-data.ts` — no `starts_at` floor, unlike `getUpcomingBookings`/the week grid's own query), for no-show tracking. `guest_unconfirmed` (still-pending guest bookings, `CountdownBadge`, `confirmBookingAsOwner` transitioning `pending_confirmation` → `confirmed`) is now reachable ONLY via admin tooling's `statusOverride` escape hatch — every normal-flow booking (guest OR patient, verified or not) is confirmed immediately now (2026-09, guest-immediate-confirm-with-clinic-followup, extended to cover sign-up-mid-booking too — see `public-booking`'s gotchas). `guest_confirmed` AND `first_time` (2026-09: first-time patients get the identical treatment — already confirmed, but still someone the clinic hasn't met) both use `clinic_reviewed_at` (nullable, `kalendar_bookings`) — a standing follow-up flag the clinic sets via `markBookingReviewedAsOwner` ("Contactado / Confirmado" button, both the Clientes-tab row and `booking-detail-modal.tsx`) once they've reached out — never auto-set, never re-derived from `clientStatus`. `needsClinicFollowUp()` (`calendar-grid-view.tsx`) is the ONE shared definition of "needs a clinic action" — used by the week/month grid's amber chip color, the dot marker's visibility, the booking-detail-modal's follow-up banner, the Clientes-tab's `needsAttentionCount` badge, AND its per-row action button — never re-derive this condition inline, always call the function, or these five surfaces WILL drift. `returning` never needs follow-up. **Labeling (2026-09)**: `CLIENT_STATUS_LABEL`/`CLIENT_STATUS_BADGE_CLASS`/`CLIENT_STATUS_DOT_CLASS` (static Records) are BASE/category labels only now — used for the Clientes-tab filter chips, where "Invitado"/"Paciente" name a category, not a specific booking's state. Any per-booking display (the badge in `booking-detail-modal.tsx`, the Clientes-tab row's badge, the grid dot) MUST call `clientStatusLabel()`/`clientStatusBadgeClass()`/`clientStatusDotClass()` instead (all in `calendar-grid-view.tsx`, all take `(clientStatus, clinicReviewedAt)`) — these append "· confirmado" / switch to amber ONLY once the specific booking's `clinicReviewedAt` is set, for `guest_confirmed`/`first_time`. This was a deliberate fix: the old static labels said "Invitado · confirmado" for EVERY guest booking (since the booking itself is always confirmed immediately — see `public-booking`), which read as "already handled" even when the clinic hadn't actually followed up yet — conflating booking-confirmation with clinic-review. Using the static maps directly on a per-booking badge is the bug to avoid. **"Unify guests and patients" (2026-09)**: `createBookingAsOwner` also now checks the typed email against existing VERIFIED patient accounts (`findVerifiedPatientIdByEmail`, `lib/booking/patient-claim.ts`) and links `patient_id` directly at creation when one matches, instead of always guest-shaping the booking — see `patient-portal`'s gotchas for the full claim-flow design (also runs on the patient side, when a guest later registers). Reminder failure visibility: `reminder_send_failed`/`last_reminder_error` (set by the reminders cron — see shared infra) are threaded through `owner-data.ts`'s `WeekViewBooking` → `app/panel/calendar/page.tsx` mapping → `calendar-grid-view.tsx`'s `WeekBookingVM` (small amber ⚠ marker on the week chip) → `booking-detail-modal.tsx` (labeled row with the error as a tooltip). Visibility-only, no retry UI or notification system.
...
- **RESOLVED (2026-07-30) — browser extensions (confirmed: Bitwarden; likely also applies to ad blockers/privacy extensions) can silently block `js.stripe.com` from loading at all**, leaving both `subscribe-modal.tsx` and `payment-method-modal.tsx` stuck indefinitely on their "Abriendo..." loading state with no error shown — confirmed via browser console: `Uncaught (in promise) Error: Failed to load Stripe.js` / `net::ERR_TIMED_OUT`. This is a real risk for actual clinic customers, not just a local dev annoyance. Fix: both modals now race `loadStripe()`'s promise against an 8s timeout (`STRIPE_LOAD_TIMEOUT_MS`) BEFORE doing anything else — checked before `createSubscriptionIntent`/`createSetupIntent` even runs, so a blocked browser doesn't waste a server-side Stripe API call (and, for the subscribe flow specifically, doesn't create an orphaned `incomplete` Stripe subscription that will never be completed — see the `createSubscriptionIntent` trade-off note above). If the timeout fires, `subscribe-modal.tsx` falls back to `createCheckoutSession` (hosted Checkout redirect); `payment-method-modal.tsx` falls back to `createBillingPortalSession` (general Customer Portal link, since there's no scoped Checkout-equivalent for a card-only update). Neither fallback requires Stripe.js to have loaded — they're plain server actions returning a URL to redirect to.
