...
- **OPEN INVESTIGATION (2026-07-29) — `createSubscriptionIntent` inconsistently fails to produce a usable client secret.** For a real, non-zero (€49) first invoice with `payment_behavior: 'default_incomplete'` + `payment_settings.save_default_payment_method: 'on_subscription'`, Stripe should return either a PaymentIntent-backed `invoice.confirmation_secret.client_secret` or, if it routes through its SetupIntent-first pattern (happens when the customer has no default payment method yet), a `subscription.pending_setup_intent`. Observed in production testing: attempt #1 got a `pending_setup_intent` object but with no usable `client_secret` even after a direct `stripe.setupIntents.retrieve()` re-fetch; attempt #2 (same flow, different subscription object, same customer) got NEITHER a `pending_setup_intent` NOR a `confirmation_secret` at all. Both invoices were `status: 'open'`, `total: 4900` — normal-looking non-zero invoices. Root cause not yet identified — candidates not yet ruled out: (a) an account-level Stripe setting/beta-opt-in gating `confirmation_secret` population, (b) genuine async finalization lag longer than the immediate re-fetch accounts for, (c) something specific to `products.create()`-per-attempt line items vs. a stable priced Product. **Mitigation shipped, not a fix:** `subscribe-modal.tsx` now automatically falls back to `createCheckoutSession` (the hosted-Checkout redirect, confirmed working) if `createSubscriptionIntent` fails, so subscribing still works end-to-end — just not always via the in-app modal as intended. Next session should reproduce with Stripe CLI (`stripe listen` + local dev, so raw API responses can be inspected directly, unlike this session which had to infer everything from Vercel log diagnostics) before attempting another fix.
