...
- **Password reset**: `/forgot-password` → `/reset-password`, via Better Auth's core `emailAndPassword.sendResetPassword`/`requestPasswordReset`/`resetPassword` (not a plugin). 1h token expiry, revokes other sessions on reset. See `MODULES.md` → `auth` for the full redirect round-trip. Clinic accounts only — patients don't have this yet.
