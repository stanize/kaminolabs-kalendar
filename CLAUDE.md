...
- **Password reset**: `/forgot-password` → `/reset-password`, via Better Auth's core `emailAndPassword.sendResetPassword`/`requestPasswordReset`/`resetPassword` (not a plugin). 1h token expiry, revokes other sessions on reset. Shared by clinic (`/signin`) and patient (`/patient/login`) accounts via `?from=patient&redirectTo=...` context-carrying — see `MODULES.md` → `auth` for the full redirect round-trip.
...
### Cron
One scheduled job (`send-reminders`); the `sweep-expired-bookings` job that used to run alongside it was removed (2026-09, see below).
...
- **`sweep-expired-bookings` — REMOVED (2026-09)**, along with `app/bookings/confirm/[token]/page.tsx` and `.github/workflows/sweep-cron.yml`. Guest bookings are confirmed immediately on submit now, no more 24h review window to sweep (see `MODULES.md` → `public-booking`'s guest-immediate-confirm-with-clinic-followup). `CRON_SECRET` now authorizes only `send-reminders`.
...
| `CRON_SECRET` | Bearer token authorizing the `send-reminders` cron route. No longer a Vercel-only auto value — as of the 2026-07-25 pg_cron migration it's a manually-rotated secret kept in sync across **three** places: this Vercel env var, the GitHub Actions repo secret (used by the fallback run), and Supabase Vault (`cron_secret`, read by the `pg_cron` job body via `vault.decrypted_secrets`). Rotate all three together or the scheduler will start getting 401s. |
...
| `WHATSAPP_CONFIG_ENCRYPTION_KEY` | Application-layer AES-256-GCM key (`lib/whatsapp/crypto.ts`) encrypting `kalendar_whatsapp_config.twilio_auth_token_encrypted` before it's stored — the key never touches SQL, only this env var. Any long random string works (e.g. `openssl rand -base64 32`); it's passed through scrypt to derive the actual 32-byte key. Losing/rotating this makes every already-saved clinic's Twilio Auth Token undecryptable — they'd need to re-enter it. See `workflows/whatsapp-booking.md`. |
