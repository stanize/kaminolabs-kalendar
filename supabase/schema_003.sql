...
--
-- Note: kalendar_support_tickets is excluded — table not yet in this DB.
--       It will get its FK when schema_002.sql is applied.
--
-- After this migration:  delete from "user" where email = '...';  → full cleanup.
--
-- NOTE on id type: Better Auth's user.id is TEXT. owner_id is already text;
-- kalendar_profiles.id was uuid (inconsistent) and is corrected to text below.
-- Verify with:
